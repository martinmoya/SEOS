...
Estado Actual (Sprint 9 Completado - v0.9.0):
...
- Siguiente paso: Sprint 10 (API & Database Generators + Mejora de /help <comando>).

Requisitos específicos para el Sprint 10:
1. API & Database Generators: Crear `ApiGenerator` y `DbGenerator` para generar endpoints REST y scripts SQL/modelos SQLAlchemy.
2. Mejora de /help: Modificar `BaseAgent` para que cada agente tenga un atributo `description`. Modificar el Kernel para que `/help <comando>` muestre la descripción y parámetros de ese agente específico.
...
Instrucción: Asimila este contexto. Si el usuario te pide iniciar el Sprint 10, entregale el plan y el código completo listo para ejecutar, cumpliendo los requisitos específicos mencionados.
